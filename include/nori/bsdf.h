...
struct BSDFQueryRecord {
	...
	/// UV coordinates correspondint to the intersection point
	Point2f uv;
	...
	/// Create a new record for sampling the BSDF
	BSDFQueryRecord(const Vector3f &wi, Point2f uv, Intersection si)
		: wi(wi), eta(1.f), uv(uv), si(si), measure(EUnknownMeasure) { }
	...
	/// Create a new record for querying the BSDF
	BSDFQueryRecord(const Vector3f &wi,
		const Vector3f &wo, Point2f uv, Intersection si, EMeasure measure)
		: wi(wi), wo(wo), eta(1.f), uv(uv), si(si), measure(measure) { }
