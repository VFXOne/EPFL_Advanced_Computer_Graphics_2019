...
struct EmitterSample {
	Point3f p; // Sampled position
	Normal3f n; // Per-vertex or face normal
	bool isVertexNormal;
	float pdf; // Probability density of the sample


	EmitterSample()
		: p(Point3f(0.f)), n(Normal3f(0.f)), isVertexNormal(true), pdf(0.f)
	{}


	EmitterSample(Point3f point, Normal3f normal, bool type, float density) 
		: p(point), n(normal), isVertexNormal(type), pdf(density)
	{}
...
struct Intersection {
    ...
    Point2f uv; // interpolated uvs
    /// Shading frame (based on the shading normal)
    Frame shFrame;
    /// Geometric frame (based on the true geometry)
    Frame geoFrame;

	/// Face index of the intersection
	uint32_t fidx; 
...
public:
	...
	/// Return a pointer to the texture tangent coordinates (or \c nullptr if there are none)
	const MatrixXf &getVextexTangent() const { return m_Tu; }

	/// Return a pointer to the texture bi-tangent coordinates (or \c nullptr if there are none)
	const MatrixXf &getVextexBiTangent() const { return m_Tv; }
	...
	/* Added functions - Homework 4 */

	float pdf() const;

	EmitterSample sampleSurface(const Point2f &sample) const; 
...
protected:
	...
	MatrixXf      m_Tu;                  ///< Vertex tangent (increasing u) coordinates (added)
	MatrixXf      m_Tv;                  ///< Vertex bi-tangent (increasing v) coordinates (added)
    MatrixXu      m_F;                   ///< Faces
